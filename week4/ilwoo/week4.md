...
init 시스템의 역사를 보면 다음과 같다:

```
Unix (1970년대) → SysVinit (Unix의 init 방식을 계승) → systemd (2010년대, 현대적 대체)
```

systemd를 이해하려면 "왜 기존 방식(SysVinit)을 바꿔야 했는가"를 알아야 하므로, SysVinit부터 살펴본다.

### 3-2. SysVinit이란?

**SysVinit**(System V init)은 1980년대 Unix System V에서 시작되어 수십 년간 리눅스의 표준 init 시스템으로 사용되었다.

**동작 방식**:
- `/etc/init.d/` 디렉터리에 있는 쉘 스크립트를 **순서대로 하나씩** 실행
- 각 스크립트가 `start`, `stop`, `restart` 같은 인자를 받아 서비스를 제어
- **런레벨(runlevel)** 개념으로 시스템 모드를 구분

> **런레벨(Runlevel)이란?** 시스템의 "동작 모드"를 숫자로 구분한 것이다. 윈도우의 "안전 모드", "일반 모드" 같은 개념이라고 생각하면 된다. 런레벨 3이면 CLI만 켜진 서버 모드, 5면 GUI까지 켜진 데스크톱 모드이다. systemd에서는 이 런레벨을 **Target**이라는 개념으로 대체했다.
...
systemd가 관리하는 모든 대상을 **유닛(Unit)**이라고 부른다. **유닛 하나 = 관리 대상 하나**이다. 예를 들어 `sshd.service`는 유닛 1개이다. 각 유닛은 `.service`, `.target` 같은 확장자를 가진 설정 파일(**유닛 파일**)로 정의된다.

유닛은 프로세스(서비스)만 관리하는 것이 아니라, systemd가 관리하는 모든 종류의 대상을 포괄하는 분류 체계이다:

| 유닛 타입 | 확장자 | 쉽게 말하면 | 예시 |
|-----------|--------|------------|------|
| **Service** | `.service` | 백그라운드 프로그램(데몬) 관리 | `sshd.service`, `crond.service` |
| **Target** | `.target` | 여러 유닛을 묶은 그룹 | `multi-user.target`, `graphical.target` |
| **Socket** | `.socket` | 요청이 올 때만 서비스를 켜는 트리거 | `cups.socket` |
| **Timer** | `.timer` | 시간 조건으로 서비스를 켜는 트리거 (cron의 systemd 버전) | `logrotate.timer` |
| **Mount** | `.mount` | 디스크 마운트를 systemd로 관리 | `home.mount` |

각 유닛 타입을 좀 더 자세히 살펴보면:

> **Service (서비스)**: 유닛의 한 종류로, 백그라운드에서 실행되는 프로그램(**데몬**)을 의미한다. SSH 서버(sshd), 방화벽(firewalld), 시간 동기화(chronyd) 등이 대표적이다. 프로세스를 직접 시작/중지/재시작하는 역할을 한다.

> **Target (타겟)**: 여러 유닛을 묶은 **그룹** 역할의 유닛이다. `multi-user.target`은 "CLI 환경에서 필요한 서비스들의 묶음"이고, `graphical.target`은 "GUI까지 포함한 서비스들의 묶음"이다. 과거의 런레벨을 대체한다. Target 자체가 프로그램을 실행하는 것은 아니고, "이 Target이 활성화되면 소속된 서비스들을 함께 시작해라"라는 의미이다.

> **Socket (소켓)**: 평소에는 서비스를 꺼두다가, 네트워크 요청이 들어오면 그때 서비스를 시작하는 **트리거** 역할이다. 예를 들어 프린터 서비스(cups)는 평소에 꺼져 있다가 인쇄 요청이 오면 자동으로 시작된다. 자원을 절약할 수 있다.

> **Timer (타이머)**: 정해진 시간에 서비스를 실행하는 **스케줄러** 역할이다. cron과 비슷하지만, systemd의 로그/의존성 관리 기능을 함께 쓸 수 있다는 장점이 있다.

> **Mount (마운트)**: 디스크 파티션을 특정 디렉터리에 연결(마운트)하는 작업을 관리한다. 3주차에서 배운 `/etc/fstab`과 비슷한 역할을 systemd가 수행하는 것이다.

> **유닛 vs 프로세스**: 프로세스는 "실행 중인 프로그램"이고, 유닛은 "systemd가 관리하는 대상"이다. Service 유닛이 프로세스를 관리하지만, Target/Socket/Timer/Mount 같은 유닛은 프로세스가 아니라 그룹/트리거/스케줄/마운트를 관리한다. 즉 유닛은 프로세스보다 넓은 개념이다.
...
Windows의 작업 관리자(Ctrl+Shift+Esc)와 거의 같은 역할을 하는 도구이다. CPU 사용률, 메모리 사용률, 실행 중인 프로세스 목록을 실시간으로 보여준다.
...
nice 값은 단순히 "순서"를 정하는 것이 아니라 **CPU 시간 배분 비율**에 영향을 준다:

- CPU가 여유로울 때: nice 값과 상관없이 모든 프로세스가 충분히 실행됨
- **CPU가 부족할 때**: nice 값이 낮은(우선순위 높은) 프로세스가 더 많은 CPU 시간을 할당받음

예를 들어, nice 0인 프로세스 A와 nice 10인 프로세스 B가 동시에 CPU를 원하면, A가 B보다 더 많은 CPU 시간을 가져간다. 즉 nice는 **"CPU가 부족할 때 누구를 먼저 배려할 것인가"**를 결정하는 값이다.
